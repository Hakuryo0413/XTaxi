...
type MapComponentProps = {
  onDistanceChange: (distance: number, pickupLocation: Location, dropoffLocation: Location) => void; // Callback to send distance and locations to parent component
};
...
const MapComponent: React.FC<MapComponentProps> = React.memo(({ onDistanceChange }) => {
  ...
  useEffect(() => {
    if (mapRef.current && !mapInstanceRef.current) {
      ...
      L.easyButton('fa-car', (btn, map) => {
        routingControl.spliceWaypoints(0, 1, map.getCenter());
      }, 'Chỉ đường từ đây').addTo(map);
